(function() {
    ...
    Ext.define('Rally.apps.board.RowSettingsField', {
        alias: 'widget.boardrowsettingsfield',
        ...
        _getRowableFields: function (models) {
            var artifactModel = Rally.domain.WsapiModelBuilder.buildCompositeArtifact(models, this.context),
                ...
                rowableFields = _.filter(allFields, function (field) {
                    ...
                    return !field.hidden &&
                        attr &&
                        (attr.Custom || attr.Constrained || attr.AttributeType.toLowerCase() === 'object') &&
        ...
        refreshWithNewModelType: function(type, context) {
            Rally.data.ModelFactory.getModels({
                types: [type],
                context: context,
                success: this._onModelsRetrieved,
