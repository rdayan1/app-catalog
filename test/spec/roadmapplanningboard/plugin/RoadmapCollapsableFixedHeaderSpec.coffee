...
describe 'Rally.apps.roadmapplanningboard.plugin.RoadmapCollapsableFixedHeader', ->
  ...
  #TODO: use @spyOnMessage Rally.clientmetrics.Message.action and inspect the description argument. For some reason
  # the autowired client metrics builder doesn't work in tests
  describe 'client metrics', ->
    it 'should log collapse message when toggle button is clicked and header is expanded', ->
      @createCardboard().then =>
        @toggleExpansion().then =>
          expect(@cardboard.plugins[0].getCickActionDescription()).toEqual("Roadmap header expansion toggled from [true] to [false]")

    it 'shouldlog expand message when expand button is clicked and header is collapsed', ->
      @createCardboard('false').then =>
        @toggleExpansion().then =>
          expect(@cardboard.plugins[0].getCickActionDescription()).toEqual("Roadmap header expansion toggled from [false] to [true]")
